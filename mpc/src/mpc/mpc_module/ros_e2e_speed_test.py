...
from .ros_adapter import DEFAULT_MODEL_CONFIG_PATH, MpcRosAdapter, _load_yaml, _section
from .ros_e2e_smoke_test import _default_states, _input_entries, _make_odom_msg, _make_point_msg


def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Measure MPC ROS adapter throughput limit.")
    parser.add_argument("--model-config", default=DEFAULT_MODEL_CONFIG_PATH)
    parser.add_argument("--duration", type=float, default=10.0)
    parser.add_argument("--warmup", type=float, default=1.0)
    parser.add_argument("--input-rate", type=float, default=200.0)
    parser.add_argument("--adapter-rate", type=float, default=1000.0)
...
def _start_adapter_thread(model_config_path: str):
    ...
    thread = threading.Thread(target=adapter.spin, name="mpc_adapter_speed_limit", daemon=True)
    thread.start()
    return thread
...
def main() -> int:
    ...
    rospy.init_node("mpc_ros_e2e_speed_limit_test", anonymous=True)
    if not args.no_start_adapter:
        limit_config_path = _write_limit_config(config, args.model_config, float(args.adapter_rate))
        rospy.loginfo("starting in-process MPC adapter with publish_rate_hz=%.2f Hz", args.adapter_rate)
    ...
    def output_callback(msg: PoseStamped):
        if collecting["enabled"]:
            output_stamps.append(float(rospy.Time.now().to_sec()))

    rospy.Subscriber(output_topic, PoseStamped, output_callback, queue_size=100)
    ...
    span = max(1e-9, output_stamps[-1] - output_stamps[0]) if sample_count >= 2 else 0.0
    measured_rate = (sample_count - 1) / span if sample_count >= 2 else 0.0
    ...
    if float(args.min_rate) > 0.0 and measured_rate < float(args.min_rate):
        ...
        return 1
    rospy.loginfo("PASS: measured MPC output limit %.3f Hz", measured_rate)
    return 0
