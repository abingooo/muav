...
void ObjPredictor::init() {
  ...
  for (int i = 0; i < obj_num_; i++) {
    ...
    ros::Subscriber pose_sub = node_handle_.subscribe<geometry_msgs::PoseStamped>(
        nsTopic("dynamic/pose_" + std::to_string(i)), 10, &ObjHistory::poseCallback, obj_his.get());
  ...
  marker_sub_ = node_handle_.subscribe<visualization_msgs::Marker>(nsTopic("dynamic/obj"), 10,
