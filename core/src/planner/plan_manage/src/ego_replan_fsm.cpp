...
namespace
{
std::string nsTopic(const std::string &topic)
{
  return ros::names::append(ros::this_node::getNamespace(), topic);
}
...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    if (planner_manager_->pp_.drone_id >= 1)
    {
      ...
      swarm_trajs_sub_ = nh.subscribe(nsTopic(sub_topic_name), 10, &EGOReplanFSM::swarmTrajsCallback, this, ros::TransportHints().tcpNoDelay());
    }
    string pub_topic_name = string("drone_") + std::to_string(planner_manager_->pp_.drone_id) + string("_planning/swarm_trajs");
    swarm_trajs_pub_ = nh.advertise<traj_utils::MultiBsplines>(nsTopic(pub_topic_name), 10);
    ...
    waypoint_done_pub_ = nh.advertise<std_msgs::String>(nsTopic("toplan/waypoint_done"), 10);

    

    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      waypoint_sub_ = nh.subscribe(nsTopic("move_base_simple/goal"), 1, &EGOReplanFSM::waypointCallback, this);
    }
    else if (target_type_ == TARGET_TYPE::PLAN_TARGET)
    {
      planpoint_sub_ = nh.subscribe(nsTopic("toplan/single_plan_point"), 1, &EGOReplanFSM::planpointCallback, this);
      yaw_sub_      = nh.subscribe(nsTopic("toplan/yaw_deg"), 1, &EGOReplanFSM::yawCallback, this);
      hover_yaw_cmd_pub_ = nh.advertise<std_msgs::Float64>(nsTopic("px4ctrl/hover_yaw_cmd"), 10);
      current_yaw_pub_ = nh.advertise<std_msgs::Float64>(nsTopic("current_yaw_deg"), 10);
    }
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      trigger_sub_ = nh.subscribe(nsTopic("traj_start_trigger"), 1, &EGOReplanFSM::triggerCallback, this);
