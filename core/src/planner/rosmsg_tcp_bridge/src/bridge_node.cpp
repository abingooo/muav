...
int main(int argc, char **argv)
{
  ...
  swarm_trajs_sub_ = nh.subscribe(nsTopic(sub_traj_topic_name), 10, multitraj_sub_tcp_cb, ros::TransportHints().tcpNoDelay());

  if ( drone_id_ >= 1 )
  {
    string pub_traj_topic_name = string("drone_") + std::to_string(drone_id_ - 1) + string("_planning/swarm_trajs");
    swarm_trajs_pub_ = nh.advertise<traj_utils::MultiBsplines>(nsTopic(pub_traj_topic_name), 10);
  }

  other_odoms_sub_ = nh.subscribe("my_odom", 10, odom_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  other_odoms_pub_ = nh.advertise<nav_msgs::Odometry>(nsTopic("others_odom"), 10);
  ...
  one_traj_sub_ = nh.subscribe(nsTopic("broadcast_bspline"), 100, one_traj_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  one_traj_pub_ = nh.advertise<traj_utils::Bspline>(nsTopic("broadcast_bspline2"), 100);
